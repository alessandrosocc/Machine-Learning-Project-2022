    ...
    def predict(self, test_x):
        if self.fitted: #posso continuare con la funzione solo se i classificatori sono già stati allenati
            ...
            for i in range(0, len(test_x)): #per ogni riga del test set viene trovata la classe prevalente tra i classificatori
                pred_y.append(aggregate_function(proba[:, i, :].T, self.voting, self.w, predictions[i]))
...
def myEnsemble(tra,mod,bil,voting,weight,peso1,peso2,peso3):
    ...
    clear_terminal()
    if voting == "Hard Voting":
        voting_type = 1
    elif voting == "Soft Voting":
        voting_type = 2
    if weight == "Non pesato":
        scelta = 2
    elif weight == "Pesato":
        scelta = 1
    if(scelta == 1): #nel caso si inserisca 1 devono essere inseriti i pesi per ogni classificatore
        ...
        weights.append(peso1) 
        weights.append(peso2) 
        weights.append(peso3) 
